...
これで、[Flask](http://flask.pocoo.org/)によるWebアプリを作ることが出来ました！

## 何が起ったのか
先程作成したプログラムが何をやったのか、逐一動きを追ってみることにします。

この節は少々退屈なので、読み飛ばして[まとめ](./#ざっくりとしたまとめ)に行ってしまっても問題ありません。
必要があれば、その都度このページに戻ってきてください。

### import flask
一番初めの行で、Flaskのインポートを行なっています。
...
最後の文は、ほとんど見た目通りの意味になります。すなわち、「'Hello World!'という文をクライアントに返す」ということです。
...
## ざっくりとしたまとめ
Flaskで作られたWebアプリは、非常にざっくりと言うと三つのステップで実行されます。
...
app = flask.Flask(__name__)
```
これは毎回同じなので、今後の解説では省略します。

### ページの定義
二つめはそれぞれのページの定義です。
何か作りたいときは、殆どの場合ここを書き換えることになります。
...
### Webサーバの実行
最後は、Webサーバの実行です。
FlaskはWebサーバを内蔵しているので、以下のプログラムを書けば単独で実行することが出来ます。
``` python
if __name__ == '__main__':
    app.run()
```
これも毎回ほとんど変わらないので、とくに必要が無ければ省略します。
